...
namespace HorrorGame
{
    ...
    public class Room : Microsoft.Xna.Framework.GameComponent
    {
        ...
        ********************************/
        ...
        private int[,] backUpRoomDesign = new int[20, 15];
        ...
        private void initClosedDoors()
        {
            /*
             * Sometimes, doors will have locks. 
             */
        }
        ...
        private void initItems()
        {
            ...
            if (rand.NextDouble() < ROOM_ITEM_SPAWN_CHANCE || roomType == DEAD_END)
            {
                for (int i = 0; i < NUMBER_OF_ITEM_SPAWNS + ( ( roomType == DEAD_END ) ? 1 : 0 ) ; i++)
                {
                    ...
                    } while (isDoor(roomDesign[x, y]) || isWall(roomDesign[x, y]) || isRedArea(roomDesign[x, y]) || roomEntitiesLocationIsUsed((x +1) * 32, y * 32, roomSpawns, 2));
        ...
        private bool isRedArea(int roomDesign)
        {
            return roomDesign == 20;
        }
        ...
        private Boolean isEntityAllowed(int entityType, int x, int y)
        {
            switch(entityType)
            {
                ...
                case Entity.BOTTLES1:
                case Entity.BOTTLES2: 
                ...
                case Entity.DEBRIS:
                case Entity.TABLE:
                case Entity.BIG_HOLE:
                case Entity.SMALL_HOLE:
                    return (x >= 0 && x < 19 && y >= 0 && y < 14 &&
                        isFloor(roomDesign[x, y]) &&
        ...
        private void initEntities()
        {
            ...
            for (int i = 0; i < NUMBER_OF_ENTITY_SPAWNS; i++)
            {
                ...
                switch (entityType)
                {
                    ...
                    case Entity.DEBRIS: current = new Entity("Debris", location, Entity.BIG, Entity.DEBRIS_INDEX); break;
                    case Entity.TABLE: current = new Entity("Table", location, Entity.BIG, Entity.TABLE_INDEX); break;
                    case Entity.BIG_HOLE: current = new Entity("Table", location, Entity.BIG, Entity.BIG_HOLE_INDEX); break;
                    case Entity.SMALL_HOLE: current = new Entity("Table", location, Entity.BIG, Entity.SMALL_HOLE_INDEX); break;
                    case Entity.BOTTLES1: current = new Entity("Table", location, Entity.SMALL, Entity.BOTTLES1_INDEX); break;
                    case Entity.BOTTLES2: current = new Entity("Table", location, Entity.SMALL, Entity.BOTTLES2_INDEX); break;
                    case Entity.PAPER: current = new Entity("Table", location, Entity.BIG, Entity.PAPER_INDEX); break;
                    case Entity.HAIR: current = new Entity("Table", location, Entity.BIG, Entity.HAIR_INDEX); break;
                }
        ...
        {
            return design >= 0 && design < 200 && design != 20 && design != 0 && design != 126 && design != 127 && design != 146 && design != 147;
        }
        ...
        private void changeToBloodRoom()
        {
            backUpRoomDesign = roomDesign;
            for (int y = 0; y < 15; y++)
            {
                for (int x = 0; x < 20; x++)
                {
                    if (isWall(roomDesign[x, y]))
                    {
                        roomDesign[x, y] = getBloodWall(roomDesign[x, y]);
                    }
                    else if (isFloor(roomDesign[x, y]))
                    {
                        roomDesign[x, y] = getBloodFloor(roomDesign[x, y]);
                    }
                }
            }
        }

        private int getBloodWall(int roomDesign)
        {
            if (roomDesign >= 200 && roomDesign < 280)
            {
                if (roomDesign >= 200 && roomDesign < 220)
                {
                    if (roomDesign % 2 == 0)
                    {
                        return 126;
                ...
                else if (roomDesign >= 220 && roomDesign < 240)
                {
                    if (roomDesign % 2 == 0)
                    {
                        return 146;
        ...
        /// <summary>
        /// TODO: This might have problems with tiles from 52-onwards.
        /// </summary>
        /// <param name="roomDesign"></param>
        /// <returns></returns>
        private int getBloodFloor(int roomDesign)
        {
            if (roomDesign % 20 != 0 && roomDesign >= 0 && roomDesign < 80)
            {
                if (roomDesign >= 0 && roomDesign < 20 || roomDesign >= 40 && roomDesign <= 60)
                {
                    if (roomDesign % 2 == 0)
                    {
                        return 120;
        ...
        /// <returns>The average of the doors, so that you end up in the center</returns>
        public Vector2 getLocationStartFrom(int dir)
        {
            ...
            result = offsetTheMove(result, dir);

            return result;
        }

        private Vector2 offsetTheMove(Vector2 result, int dir)
        {
            switch (dir)
            {
                case 0:
